...
)
{
	...
	size_t resp_size = sizeof(struct virtio_video_resp) + (sync ? size : 0);
	...
	if (sync) {
		...
		if (ret) {
			v4l2_err(&vvd->v4l2_dev, "%s sync cmd failed: cmd_type is %s and sub_cmd_type is %s\n",
			__func__, cmd_to_string(cmd_type), cmd_to_string(sub_cmd_type));
		} else {
			v4l2_info(&vvd->v4l2_dev, "%s sync cmd done: cmd_type is %s and sub_cmd_type is %s\n",
			__func__, cmd_to_string(cmd_type), cmd_to_string(sub_cmd_type));
			resp = (struct virtio_video_resp*)vbuf->resp_buf;
			resp_payload = (void*)resp + sizeof(*resp);
			if (resp->result >= VIRTIO_VIDEO_RESP_ERR_INVALID_OPERATION)
				ret = -EINVAL;
			else
				memcpy(payload, resp_payload, size);
		}
		...
		if (ret) {
			v4l2_err(&vvd->v4l2_dev, "%s async cmd failed: cmd_type is %s and sub_cmd_type is %s\n",
			__func__, cmd_to_string(cmd_type), cmd_to_string(sub_cmd_type));
		} else {
			v4l2_info(&vvd->v4l2_dev, "%s async cmd done: cmd_type is %s and sub_cmd_type is %s\n",
			__func__, cmd_to_string(cmd_type), cmd_to_string(sub_cmd_type));
		}
	}
...
int virtio_video_cmd_g_fmt(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_format* f)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
...
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_SET_PARAMS, S_FMT, (void *)f, sizeof(*f), NULL);
}

int virtio_video_cmd_g_parm(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_streamparm* a)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_GET_PARAMS, G_PARAM,
		(void *)a, sizeof(*a), NULL);
}
...
int virtio_video_cmd_g_ctrl(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_control* c)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_GET_CONTROL, G_CTRL,
		(void *)c, sizeof(*c), NULL);
}
...
int virtio_video_cmd_g_selection(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_selection* s)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_GET_PARAMS, G_SELECTION,
		(void *)s, sizeof(*s), NULL);
}
...
int virtio_video_cmd_querybuf(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_buffer* b)
{
...
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_QUERY_CAPABILITY, QUERYCAP,
...
int virtio_video_cmd_reqbufs(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, struct v4l2_requestbuffers* b)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_GET_PARAMS, REQBUFS,
		(void *)b, sizeof(*b), NULL);
}
...
int virtio_video_cmd_streamon(struct virtio_video_device* vvd,
	struct virtio_video_stream* stream, enum v4l2_buf_type i)
{
	return virtio_video_v4l2_to_hab_sync(vvd, stream->stream_id,
		VIRTIO_VIDEO_CMD_STREAMON, 0,
		(void *)&i, sizeof(i), NULL);
}
