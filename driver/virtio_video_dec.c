...
int virtio_video_dec_init_ctrls(struct virtio_video_stream *stream)
{
	...
	list_for_each_entry(entry, &vvd->ctrl_config_list, ctrls_list_entry) {
		...
		vpr_h(strm2tag(stream),"%s: add ctrl, id=%#x, type=%#x, flags=%#x, max=%#x, min=%#x, step=%#x, def=%#x, name=%s, is_private=%d\n",
		...
		if (stream->ctrl_handler.error) {
			vpr_e(strm2tag(stream),"%s: failed to add ctrl, id=%#x, type=%#x, flags=%#x, max=%#x, min=%#x, step=%#x, def=%#x, name=%s, is_private=%d\n",
