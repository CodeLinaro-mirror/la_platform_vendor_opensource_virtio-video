...
void virtio_video_buf_queue(struct vb2_buffer *vb)
{
	...
	if (ret) {
		vpr_e(vvd2tag(vvd), "failed to queue buffer\n");
		return;
	}
			  ...
			  struct v4l2_capability *cap)
{
	...
	if (strscpy(cap->driver, DRIVER_NAME, sizeof(cap->driver)) < 0)
		vpr_e(strm2tag(stream), "failed to copy driver name\n");
	if (strscpy(cap->card, video_dev->name, sizeof(cap->card)) < 0)
		vpr_e(strm2tag(stream), "failed to copy card name\n");
				   ...
				   struct virtio_video_stream *stream)
{
	...
	if (vvd->is_m2m_dev) {
		...
		if (ret) {
			vpr_e(strm2tag(stream), "failed to get stream in params\n");
			goto err_get_parms;
		}
	}
	...
	if (ret)
		vpr_e(strm2tag(stream), "failed to get stream out params\n");
				     ...
				     struct virtio_video_stream *stream)
{
	...
	if (ret) {
		vpr_e(strm2tag(stream), "failed to get stream profile\n");
		goto err_get_ctrl;
	}
	...
	if (ret) {
		vpr_e(strm2tag(stream), "failed to get stream level\n");
		goto err_get_ctrl;
	}
	...
	if (ret)
		vpr_e(strm2tag(stream), "failed to get stream bitrate\n");
			 ...
			 struct v4l2_selection *sel)
{
	...
	case VIRTIO_VIDEO_DEVICE_CAMERA:
		...
		break;
	default:
		vpr_e(strm2tag(stream), "unsupported device type\n");
		return -EINVAL;
	}
	...
	case V4L2_SEL_TGT_CROP:
		...
		break;
	default:
		vpr_e(strm2tag(stream), "unsupported/invalid selection target: %d\n",
		      sel->target);
		return -EINVAL;
	}
...
static int virtio_video_device_open(struct file *file)
{
	...
	if (ret) {
		vpr_e(vvd2tag(vvd), "device already in use.\n");
		return ret;
	}
#ifndef VIRTIO_VIDEO_MSM
	default_fmt = list_first_entry_or_null(vvd->ops->get_fmt_list(vvd),
					       ...
					       formats_list_entry);
	if (!default_fmt) {
		vpr_e(vvd2tag(vvd), "device failed to start\n");
		ret = -EIO;
		goto err;
	}
	...
	if (ret) {
		vpr_e(strm2tag(stream), "failed to create stream\n");
	...
	if (video_dev->ctrl_handler) {
		vpr_e(strm2tag(stream), "%s: ctrl_handler is not NULL\n",
		      __func__);
	...
	if (vvd->ops->init_ctrls) {
		...
		if (ret) {
			vpr_e(strm2tag(stream), "failed to init controls\n");
			goto err_init_ctrls;
		}
	}
	return 0;
...
#endif
	if (ret) {
		vpr_e(vvd2tag(vvd), "failed to register video device\n");
		return ret;
	}

	vpr_h(vvd2tag(vvd), "Device '%s' registered as /dev/video%d\n",
	      vd->name, vd->num);
				       ...
				       void *resp_buf, bool *is_end)
{
	...
	while (num_descs--) {
		...
		if (!config->size) {
			*is_end = true;
			vpr_h(vvd2tag(vvd), "%s: got all ctrls from BE",
			      __func__);
			break;
		}
			      ...
			      enum virtio_video_queue_type queue_type)
{
	...
	while (!is_end) {
		ret = virtio_video_cmd_query_capability(vvd, resp_buf,
		                                        ...
		                                        queue_type);
		if (ret) {
			vpr_e(vvd2tag(vvd), "%s: failed to query capability",
			      __func__);
			break;
		}
		...
		if (ret) {
			vpr_e(vvd2tag(vvd), "%s: failed to parse controls",
			      __func__);
	...
	ret = virtio_video_cmd_query_capability(vvd, resp_buf, resp_size,
						queue_type);
	if (ret)
		vpr_e(vvd2tag(vvd), "failed to query capability\n");
#endif
...
int virtio_video_device_init(struct virtio_video_device *vvd)
{
	...
	ret = virtio_video_query_capability(vvd, output_resp_buf,
					    VIRTIO_VIDEO_QUEUE_TYPE_OUTPUT);
	if (ret) {
		vpr_e(vvd2tag(vvd), "failed to get output caps\n");
		goto err_output_cap;
	}
	...
	if (vvd->is_m2m_dev) {
		...
		ret = virtio_video_query_capability(vvd, input_resp_buf,
						VIRTIO_VIDEO_QUEUE_TYPE_INPUT);
		if (ret) {
			vpr_e(vvd2tag(vvd), "failed to get input caps\n");
			goto err_input_cap;
		}
		...
		if (IS_ERR(m2m_dev)) {
			vpr_e(vvd2tag(vvd), "failed to init m2m device\n");
			ret = PTR_ERR(m2m_dev);
			goto err_m2m_dev;
		}
		vfl_dir = VFL_DIR_M2M;
	...
	ret = virtio_video_parse_virtio_capabilities(vvd, input_resp_buf,
						     output_resp_buf);
	if (ret) {
		vpr_e(vvd2tag(vvd), "failed to parse a function\n");
		goto parse_cap_err;
	}
